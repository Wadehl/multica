...
func (c *codexClient) handleEvent(msg map[string]any) {
	...
	case "task_started":
		if c.onMessage != nil {
			c.onMessage(Message{Type: MessageStatus, Status: "running", SessionID: c.activeThreadID()})
		}
	case "agent_message":
...
func (c *codexClient) isNotificationFromOtherThread(params map[string]any) bool {
	...
	return ok && activeThreadID != "" && threadID != activeThreadID
}
...
func scanCodexSessionUsage(startTime time.Time, codexHome, threadID string, resumed bool) *codexSessionUsage {
	...
	if result == nil || (result.planLimits == nil && result.usage.InputTokens == 0 && result.usage.OutputTokens == 0 &&
