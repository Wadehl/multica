...
func TestRecordHeartbeatLeaseThrottlesDBScheduling(t *testing.T) {
	...
	lease := daemonws.NewRuntimeLease("workspace-1", "codex", "online", time.Now().Add(-2*runtimeHeartbeatDBFlushInterval), true)
...
func TestRecordHeartbeatLeaseScheduleFailureKeepsStaleWatermark(t *testing.T) {
	...
	lease := daemonws.NewRuntimeLease("workspace-1", "codex", "online", stale, true)
...
func TestRecordHeartbeatLeaseOfflineTransitionIsSynchronous(t *testing.T) {
	...
	lease := daemonws.NewRuntimeLease(testWorkspaceID, "codex", "offline", time.Now(), true)
